...
export default function ContactsPage() {
  ...
  const loadContacts = useCallback(async () => {
    setIsLoading(true);
    try {
      // hydrateContacts handles its own error logging and notifications internally
  ...
  const handlePreviewImport = async () => {
    ...
    setIsImporting(true);
    try {
      const response = await apiClient.post('/contacts/import/preview', buildImportPayload());
  ...
  const handleCommitImport = async () => {
    ...
    setIsImporting(true);
    try {
      const response = await apiClient.post(`/contacts/import/${importSource}`, buildImportPayload());
