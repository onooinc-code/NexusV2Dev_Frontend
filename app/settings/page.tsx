...
export default function SettingsPage() {
  ...
  const loadSettings = async () => {
    ...
    try {
      ...
      setEditedValues(
        (Object.values(data).flat() as SettingEntry[]).reduce(
          (acc: Record<string, any>, entry: SettingEntry) => {
    ...
    <AppLayout>
      <div className="p-6 h-full flex flex-col gap-6 w-full max-w-7xl mx-auto overflow-y-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-4">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">
              Manage application configuration, integrations, health monitoring, and system
              controls.
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-2 border-b border-white/10 pb-3">
          {(
            ['general', 'integrations', 'health', 'api-tester', 'seeds', 'advanced'] as TabType[]
          ).map((tab) => (
            <button
              key={tab}
              onClick={() => {
          ...
          <div className="grid grid-cols-1 gap-6">
            {loading ? (
              ...
              <NxGlassCard className="p-6 text-center text-gray-400">
                No settings are available. Please verify backend configuration.
              </NxGlassCard>
            ) : (
                  ...
                  <NxGlassCard className="p-6 bg-black/40 border-white/10">
                    <div className="mb-4 flex items-center justify-between gap-4">
                      ...
                      <span className="text-xs text-gray-400">
                        {groupedSettings.integrations?.length || 0} keys
                      </span>
                    </div>
                                  ...
                                  <button
                                    onClick={() => {
                                      ...
                                      if (!editingEncrypted[setting.key]) {
                                        ...
                                        handleValueChange(
                                          setting.key,
                                          groupedSettings.integrations?.find(
                                            (s) => s.key === setting.key
                                          )?.value
                                        );
                                      }
        ...
        {/* API Tester Tab */}
        {activeTab === 'api-tester' && <ApiTesterPanel />}
          ...
          <div className="grid grid-cols-1 gap-6">
            <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4">
              <div className="flex gap-2 text-yellow-200">
                ...
                <p className="text-sm">
                  Database seeders populate the database with test data and workflow templates. Use
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-red-300 mt-1">
                    These operations can significantly impact the system. Use only if you know what
                    you&apos;re doing.
                  </p>
                </div>
              </div>
            ...
            <NxGlassCard className="p-6 bg-black/40 border border-yellow-500/20">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    Emergency control to pause all agents system-wide. Use in case of critical
                    issues.
                  </p>
                  <p
                    className={`text-xs mt-2 ${agentPausedEnabled ? 'text-red-300' : 'text-gray-500'}`}
                  >
        ...
        {(activeTab === 'general' || activeTab === 'integrations') && (
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between border-t border-white/5 pt-4">
            <div className="text-sm text-gray-400">
              Changes are stored to the backend settings registry and will be reflected across the
              Nexus workspace on save.
            </div>
