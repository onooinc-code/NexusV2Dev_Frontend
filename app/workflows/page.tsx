...
export default function WorkflowsPage() {
  ...
  const fetchWorkflows = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await apiClient.get('/workflows?limit=50');
  ...
  useEffect(() => {
    ...
    channel.listen('.workflow.started', (event: any) => {
      setRealtimeLogs((prev) => [
        `▶ Workflow started — execution: ${event.execution_id ?? 'unknown'}`,
        ...prev,
      ]);
    ...
    channel.listen('.workflow.step_completed', (event: ReverbStepEvent) => {
      ...
      setRealtimeLogs((prev) => [
        `${statusIcon} ${event.step_name ?? event.step_id} — ${event.status}${durationStr}${event.error ? ` | Error: ${event.error}` : ''}`,
        ...prev,
      ]);
    ...
    channel.listen('.workflow.completed', (event: any) => {
      setRealtimeLogs((prev) => [`✅ Workflow completed successfully`, ...prev]);
  ...
  const createWorkflow = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // Ensure the frontend receives a version number optimistically
  ...
  const executeWorkflow = async () => {
    ...
    try {
      ...
      console.error(error);
      setRealtimeLogs([`❌ Failed to launch execution: ${error.message}`]);
  ...
  const resumeExecution = async (decision: 'approve' | 'deny') => {
    if (!execution) return;
    const response = await apiClient.post(`/workflows/executions/${execution.id}/resume`, {
      decision,
      ...
      <div className="p-6 h-full flex flex-col gap-6 w-full max-w-7xl mx-auto overflow-y-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            >
              <Radio
                className={`w-3 h-3 ${connectionStatus === 'connected' ? 'animate-pulse' : ''}`}
              />
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[280px_1fr_320px] gap-4 min-h-[520px]">
              ...
              {workflows.map((workflow) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-500 mt-1 uppercase">
                    {workflow.trigger_type} / {workflow.status}
                  </div>
                </button>
              ))}
          ...
          <NxGlassCard className="p-4 bg-white/5 border-white/5 flex flex-col">
            ...
            <div className="mt-3 text-xs font-mono text-gray-400 space-y-2 max-h-[300px] overflow-y-auto flex-1">
              ...
              {realtimeLogs.length > 0 && (
                <div className="space-y-1 mb-3">
                  ...
                  <div className="border-t border-white/5 pt-2 mt-2 text-gray-600 text-[10px]">
                    ── API step logs ──
                  </div>
                </div>
              )}
              ...
              {execution ? (
                <>
                  ...
                  {(execution.step_logs ?? []).map((log) => (
                    <div
                      key={`${log.step_id}-${log.status}`}
                      className="rounded border border-white/10 bg-black/30 p-2"
                    >
                      <div className="text-gray-200">{log.step_name}</div>
                      <div
                        className={`${log.status === 'failed' ? 'text-red-400' : log.status === 'completed' ? 'text-emerald-400' : 'text-gray-400'}`}
                      >
                        {log.status}
            ...
            {execution?.status === 'paused' &&
              execution.runtime_state?.waiting_for?.type === 'approval' && (
          ...
          <form onSubmit={createWorkflow} className="flex flex-col gap-4">
            <div>
              ...
              <NxInput
                ...
                placeholder="Daily approval workflow"
              />
            </div>
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-gray-400 mb-1.5">
