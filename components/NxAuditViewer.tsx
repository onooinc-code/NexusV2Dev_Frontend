...
export const NxAuditViewer: React.FC<NxAuditViewerProps> = ({ contactId, contactName }) => {
  ...
  const fetchAuditLogs = useCallback(async () => {
    ...
    setError('');
    try {
      const response = await apiClient.get(`/contacts/${contactId}/audit`, {
        params: { per_page: 50 },
      });
      const data =
        (response.data as { data?: { data?: AuditEvent[] } }).data?.data ??
    ...
    <div className="space-y-6">
      ...
      <NxGlassCard className="p-6 border-nexus-blue/20">
        ...
        <p className="text-sm text-gray-400 mb-6">
          ...
          data associated with this contact, including messages, memories, analysis findings, and
          audit logs.
        </p>
