...
export const NxMemoryMaintenanceModal: React.FC<NxMemoryMaintenanceModalProps> = ({
  isOpen,
  onClose,
  contactId,
}) => {
  ...
  const handleRunMaintenance = async () => {
    ...
    try {
      const response = await apiClient.post(`/contacts/${contactId}/memory-maintenance`, {
        operation,
        dry_run: isDryRun,
      });
        ...
        <div className="p-6 relative z-10">
          ...
          <div className="space-y-6">
            <p className="text-sm text-gray-300">
              Select an administrative operation to run against this contact&apos;s relationship
              database.
            </p>
            ...
            <div className="space-y-2">
              <label className="flex items-start gap-3 p-3 rounded-lg border border-white/5 cursor-pointer bg-white/5 hover:bg-white/10 transition-colors">
                <input
                  type="radio"
                  checked={operation === 'prune_stale'}
                  onChange={() => setOperation('prune_stale')}
                  className="mt-1 bg-transparent border-nexus-blue text-nexus-blue"
                />
                <div className="flex flex-col">
                  <span className="text-sm font-medium text-gray-200">Prune Stale Memories</span>
                  <span className="text-[10px] text-gray-500">
                    Deletes memories older than 1 year that haven&apos;t been validated recently.
                  </span>
                </div>
              </label>
              ...
              <label className="flex items-start gap-3 p-3 rounded-lg border border-white/5 cursor-pointer bg-white/5 hover:bg-white/10 transition-colors">
                <input
                  type="radio"
                  checked={operation === 'resolve_duplicates'}
                  onChange={() => setOperation('resolve_duplicates')}
                  className="mt-1 bg-transparent border-nexus-blue text-nexus-blue"
                />
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] text-gray-500">
                    Attempts to merge duplicate identifiers and memories automatically.
                  </span>
                </div>
              </label>
              ...
              <label className="flex items-start gap-3 p-3 rounded-lg border border-error/20 cursor-pointer bg-error/5 hover:bg-error/10 transition-colors">
                <input
                  type="radio"
                  checked={operation === 'erase_data'}
                  onChange={() => setOperation('erase_data')}
                  className="mt-1 bg-transparent border-error text-error"
                />
                <div className="flex flex-col">
                  <span className="text-sm font-medium text-error">Erase Interaction Data</span>
                  <span className="text-[10px] text-error/70">
                    Permanently purges all messages, memories, and logs (leaves base profile
                    intact).
                  </span>
                </div>
              </label>
            ...
            <label className="flex items-center gap-2 cursor-pointer mt-4">
              <input
                ...
                className="rounded border-gray-600 bg-transparent text-nexus-blue focus:ring-nexus-blue"
              />
              <span className="text-sm text-nexus-blue font-mono">
                Dry Run (Simulate only, no DB changes)
              </span>
            </label>
            ...
            {success && results && (
              <div className="bg-nexus-blue/10 border border-nexus-blue/20 p-4 rounded-lg flex flex-col gap-2 text-nexus-blue">
                <h4 className="text-sm font-bold">
                  Operation Completed {results.dry_run ? '(Dry Run)' : ''}
                </h4>
