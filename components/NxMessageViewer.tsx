...
export const NxMessageViewer: React.FC<NxMessageViewerProps> = ({
  contactId,
  contactName,
  contactAvatar,
}) => {
    ...
    const fetchMessages = async () => {
      ...
      try {
        ...
        if (channelFilter !== 'all') {
          queryParams.append(
            'channel',
            channelFilter === 'facebook' ? 'facebook_messenger' : channelFilter
          );
        }
        if (searchQuery) {
        ...
        const response = await fetch(
          `http://localhost:8000/api/v1/contacts/${contactId}/messages?${queryParams.toString()}`,
          {
            headers: {
