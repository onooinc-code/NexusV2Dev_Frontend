...
export const NxRulesViewer: React.FC<NxRulesViewerProps> = ({ contactId }) => {
  ...
  const handleAddRule = () => {
    ...
    setRules([...rules, { id: nextIdRef.current++, text: newRule }]);
    setNewRule('');
  };
  ...
  const acceptSuggestion = (suggestion: any) => {
    setRules([...rules, { id: nextIdRef.current++, text: suggestion.text }]);
        ...
        <div className="mb-8 space-y-3">
          ...
          <div className="grid gap-3">
            {suggestions.map((suggestion) => (
              <div
                key={suggestion.id}
                className="flex items-center justify-between p-3 rounded-lg bg-nexus-blue/5 border border-nexus-blue/20"
              >
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="p-1.5 bg-green-500/20 text-green-400 rounded hover:bg-green-500/40 transition-colors"
                  >
                    <Check className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => rejectSuggestion(suggestion.id)}
                    className="p-1.5 bg-error/20 text-error rounded hover:bg-error/40 transition-colors"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
        ...
        {isLoading ? (
          ...
          <ul className="space-y-2">
            {rules.map((rule) => (
              <li
                key={rule.id}
                className="flex items-center justify-between p-3 rounded-lg bg-black/20 border border-white/5 group"
              >
            ...
            {rules.length === 0 && (
              <p className="text-sm text-gray-500 text-center py-4">
                No active rules for this contact.
              </p>
            )}
          </ul>
        )}
