...
import React, { useEffect, useState, useRef, Suspense, useMemo, useLayoutEffect } from 'react';
...
function StatusBarContent({ className }: { className?: string }) {
  ...
  const pageName = useMemo(() => {
    ...
    const match = Object.keys(pageNameMap).find(
  ...
  const refreshStatus = async () => {
    ...
    try {
      const stats = await apiClient.get('/tasks/stats');
  ...
  return (
    <div
      className={cn(
        'h-10 bg-surface-dark border-t border-white/10 flex items-center justify-between px-4 text-xs font-mono text-gray-400 shrink-0 relative overflow-hidden',
        className
      )}
    >
      {navLoading ? (
        ...
        <div className="flex items-center gap-3 min-w-[240px]">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[10px] text-gray-400 truncate">
              {statusError
                ? `Health error: ${statusError}`
                : health.status === 'healthy'
                  ? `All systems nominal · ${health.timestamp}`
      ...
      <NxModal
        ...
        title={<span>Status Details</span>}
        size="lg"
      >
        <div className="space-y-4 text-xs text-gray-300">
          ...
          <div>
            ...
            <pre className="rounded-2xl bg-black/80 p-4 border border-white/10 overflow-x-auto text-[11px] text-gray-300">
              {JSON.stringify({ health: health.details, taskStats }, null, 2)}
            </pre>
          </div>
        </div>
      </NxModal>
      ...
      <NxModal
        ...
        title={<span>System Log Statistics</span>}
        size="lg"
      >
        <div className="space-y-4 text-xs text-gray-300">
          {!logsText ? (
            ...
            <div className="text-center py-8 text-red-400">{logsText}</div>
          ) : (
            <>
              {(() => {
                try {
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="rounded-2xl bg-black/70 p-4 border border-red-500/20">
                          <div className="text-[10px] text-red-400 uppercase tracking-[0.25em] mb-2">
                            Errors Today
                          </div>
                      ...
                      {data.by_level && Object.keys(data.by_level).length > 0 && (
                        <div className="rounded-2xl bg-black/70 p-4 border border-white/10">
                          <div className="text-[10px] text-gray-500 uppercase tracking-[0.25em] mb-3">
                            Logs by Level
                          </div>
                            ...
                            {Object.entries(data.by_level).map(([level, count]: any) => (
                              <div key={level} className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="font-semibold text-gray-200 w-8 text-right">
                                    {count}
                                  </span>
                                </div>
                              </div>
                      ...
                      <details className="rounded-2xl bg-black/70 p-4 border border-white/10">
                        ...
                        <pre className="mt-3 text-[10px] overflow-x-auto text-gray-300">
