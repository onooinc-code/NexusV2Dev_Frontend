...
export const NxTaskExecutionLog: React.FC<NxTaskExecutionLogProps> = ({ taskId }) => {
  ...
  return (
    <div className="bg-gray-900 text-green-400 font-mono text-sm p-4 rounded-md overflow-y-auto max-h-64 mt-4 shadow-inner border border-gray-700 animate-fade-in">
      <div className="mb-2 text-xs text-gray-500 border-b border-gray-700 pb-1">
          ...
          <div key={log.id || idx} className="py-1">
            <span className="text-gray-500">
              ...
              {log.time || log.created_at
                ? new Date(log.time || log.created_at).toLocaleTimeString()
                : ''}
              ]
            </span>
            <span
              className={`ml-2 ${log.level === 'error' || log.status === 'failed' || log.level === 'failed' ? 'text-red-400' : log.level === 'warning' ? 'text-yellow-400' : 'text-green-400'}`}
            >
