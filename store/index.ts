...
export const useGlobalStore = create<GlobalState>((set, get) => ({
  ...
  fetchContactNotes: async (id) => {
    try {
      const response = await apiClient.get(`/contacts/${id}/notes`);
  ...
  addContactNote: async (id, data) => {
    try {
      await apiClient.post(`/contacts/${id}/notes`, data);
  ...
  deleteContactNote: async (id, noteId) => {
    try {
      await apiClient.delete(`/contacts/${id}/notes/${noteId}`);
      await get().fetchContactNotes(id);
  ...
  addContactRelationship: async (contactId, data) => {
    try {
      await apiClient.post(`/contacts/${contactId}/relationships`, data);
  ...
  deleteContactRelationship: async (contactId, relationshipId) => {
    try {
      await apiClient.delete(`/contacts/${contactId}/relationships/${relationshipId}`);
      await get().fetchContactDetails(contactId);
  ...
  addContactPreference: async (contactId, data) => {
    try {
      await apiClient.post(`/contacts/${contactId}/preferences`, data);
      await get().fetchContactDetails(contactId);
  ...
  updateContactPreference: async (contactId, prefId, data) => {
    try {
      await apiClient.put(`/contacts/${contactId}/preferences/${prefId}`, data);
      await get().fetchContactDetails(contactId);
  ...
  deleteContactPreference: async (contactId, prefId) => {
    try {
      await apiClient.delete(`/contacts/${contactId}/preferences/${prefId}`);
      await get().fetchContactDetails(contactId);
  ...
  addContactAlias: async (contactId, data) => {
    try {
      await apiClient.post(`/contacts/${contactId}/aliases`, data);
      await get().fetchContactDetails(contactId);
  ...
  deleteContactAlias: async (contactId, aliasId) => {
    try {
      await apiClient.delete(`/contacts/${contactId}/aliases/${aliasId}`);
      await get().fetchContactDetails(contactId);
  ...
  hydrateTasks: async () => {
    ...
    try {
      ...
      const tasks: Task[] = (Array.isArray(data) ? data : []).map((t: any) => ({
        ...
        priority: t.priority === 10 ? 'high' : t.priority === 5 ? 'medium' : t.priority === 1 ? 'low' : 'medium',
  ...
  createTask: async (data) => {
    ...
    try {
      const payload = {
        ...
        due_date: (data.dueDate === 'TBD' || data.dueDate === 'Tomorrow' || !data.dueDate) ? null : data.dueDate,
      };
  ...
  updateTask: async (id, status) => {
    ...
    const updated = previousTasks.map(t => t.id === id ? { ...t, status } : t);
    set({ tasks: updated });
    ...
    try {
      ...
      get().addNotification('error', 'Failed to update task status');
      // Rollback
  ...
  deleteTask: async (id) => {
    ...
    set({ tasks: updated });
    get().addNotification('info', `Archiving task...`);
    
    try {
      await apiClient.delete(`/tasks/${id}`);
    } catch (error) {
      logError('Failed to delete task', error);
      get().addNotification('error', 'Failed to delete task');
      // Rollback
      set({ tasks: previousTasks });
    }
  },
  ...
  simulateAgent: async (id, payload) => {
    try {
      ...
      const response = await apiClient.post(`/agents/${id}/simulate`, body);
  ...
  runAgent: async (id, payload) => {
    try {
      ...
      const response = await apiClient.post(`/agents/${id}/run`, body);
  ...
  quarantineAgent: async (id, reason) => {
    try {
      await apiClient.post(`/agents/${id}/quarantine`, { reason });
      set((state) => ({
  ...
  hydratePersonas: async () => {
    ...
    try {
      const response = await apiClient.get('/agent-personas');
      const raw = response.data?.data ?? response.data;
      const list = Array.isArray(raw) ? raw : (Array.isArray(raw?.data) ? raw.data : []);
  ...
  createPersona: async (data) => {
    try {
      const response = await apiClient.post('/agent-personas', data);
  ...
  updatePersona: async (id, data) => {
    try {
      const response = await apiClient.put(`/agent-personas/${id}`, data);
      const apiPersona = response.data?.data ?? response.data;
      set((state) => ({
  ...
  deletePersona: async (id) => {
    try {
      await apiClient.delete(`/agent-personas/${id}`);
  ...
  connectMCPServer: async (name) => {
    try {
      await apiClient.post(`/mcp-servers/${name}/connect`);
  ...
  disconnectMCPServer: async (name) => {
    try {
      await apiClient.post(`/mcp-servers/${name}/disconnect`);
